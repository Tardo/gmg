# Original code from: https://github.com/emdemir/Flask-Babel-JS
# Modified to patch the source to work with semicolon in 'plural-form'
import gettext
import json
        ...
        if metadata:
            for m in metadata.splitlines():
                if m.lower().startswith('plural-forms:'):
                    ...
                    m_text = m.lower().split('plural=')[1]
                    if m_text[-1] == ';':
                        m_text = m_text[:-1]
                    js.append(c2js(m_text))
